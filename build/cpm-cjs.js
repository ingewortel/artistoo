...
class ConnectivityConstraint extends HardConstraint {
			...
			while( q.length > 0 ){
				...
				for( let i = 0 ; i < ne.length ; i ++ ){
					if( C.pixti( ne[i] ) == cellid &&
						!(ne[i] in visited) && (ne[i] in cbpobject) ){
						q.push(ne[i]);
						visited[ne[i]]=1;
					}
				}
	...
	*/
	checkConnected( tgt_i, src_type, tgt_type ){
		...
		let length_before = this.connectedComponentsOfCellBorder( tgt_type ).length;
	
		// Update the borderpixels as if the change occurs
		this.updateBorderPixels( tgt_i, tgt_type, src_type );
		let length_after = this.connectedComponentsOfCellBorder( tgt_type ).length;
